/*題目1. 修改上面的程式碼，建立一個10*3整數陣列、一個10元素的福點數數陣列
以及一個指標陣列name，將全班10人的學號，數學成績，計概成績放入整數陣列中
(可直接置於程式內)，將全班10人的姓名放入指標陣列中。計算出每個學生的平均
成績放入浮點數陣列中，並根據每個學生的平均分數，由大到小排序。需使用副程
式與傳位址呼叫進行資料交換，字串交換副程式需使用p.333的拷貝副程式完成。*/
#include <stdio.h>
#include <string.h>
#define SIZE 10
