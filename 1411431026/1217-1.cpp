...
void sort_array(int stu[][SIZE], float avg[], int n) {
    int i, j, temp;
    float temp1;
...
int main() {
    ...
    for (i = 0; i < SIZE; i++) {
        printf("%d\t%d\t%d\t%.1f\n",
            stu[0][i], stu[1][i], stu[2][i], average_scores[i]);
    }

    sort_array(stu, average_scores, SIZE);

    
    float mode = find_mode(average_scores, SIZE);
